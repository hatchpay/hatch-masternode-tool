...
from hatchd_intf import HatchdIndexException, Masternode
...
# will display data from cache, instead of requesting them from a hatch daemon, which is
# more time consuming.
...
log = logging.getLogger('hmt.proposals')
...
class VotingMasternode(AttrsProtected):
    def __init__(self, masternode, masternode_config):
        ...
        :param masternode: ref to an object storing mn information read from the network (hatchd_intf.Masternode)
...
class Proposal(AttrsProtected):
    ...
    def apply_values(self, masternodes, last_superblock_time, next_superblock_datetime):
        ...
        if not self.get_value('title'):
            # if title value is not set (it's an external attribute, from hatchcentral) then copy value from the
            # name column
...
class ProposalsDlg(QDialog, ui_proposals.Ui_ProposalsDlg, wnd_utils.WndUtils):
    def __init__(self, parent, hatchd_intf):
        ...
        for idx, mn in enumerate(self.app_config.masternodes):
            ...
            if mn_ident not in mn_idents:
                if mn.hmn_voting_key_type == InputKeyType.PRIVATE:
                    voting_key = mn.get_current_key_for_voting(self.app_config, self.hatchd_intf)
                    if voting_key:
                        if hatch_utils.validate_wif_privkey(voting_key, self.app_config.hatch_network):
    ...
    def display_budget_summary(self):
        ...
        if self.next_budget_approved is not None:
            budget_approved = f'<td><b>Budget approved:</b> {app_utils.to_string(round(self.next_budget_approved))} Hatch '
        ...
        if self.next_budget_requested is not None:
            budget_requested = f'<td><b>Budget requested:</b> {app_utils.to_string(round(self.next_budget_requested))} Hatch '
        ...
        if self.votes_loaded:
            if self.next_budget_approved_by_user_yes_votes is not None:
                budget_approved_user_yes = \
                    ...
                    f'{app_utils.to_string(round(self.next_budget_approved_by_user_yes_votes))} Hatch '
                if self.next_budget_amount:
                    budget_approved_user_yes += \
        ...
        message = '<html><head></head><style>td{padding-right:10px;white-space:nowrap;}</style><body>' \
                  ...
                  f'{bra}<tr><td><b>Budget available:</b> {app_utils.to_string(round(self.next_budget_amount))} Hatch</td>' \
                  f'<td><b>Budget left:</b> {app_utils.to_string(round(self.next_budget_left))} Hatch</td></tr>' \
    ...
    def read_proposals_from_network(self):
        """ Reads proposals from the Hatch network. """
        ...
        try:
            ...
            log.info('Reading proposals from the Hatch network.')
            begin_time = time.time()
            proposals_new = self.hatchd_intf.gobject("list", "valid", "proposals")
            ...
            if len(proposals_new) > 0:
                if errors < len(proposals_new)/10:
                    try:
                        ...
                        for prop in self.proposals:
                            ...
                            if prop.marker:
                                if not prop.db_id:
                                    ...
                                    # hatchd sometimes does not return some proposals, so they are deactivated id the db
                                    ...
                                    if row:
                                        prop.db_id = row[0]
                                        prop.modified = True
                                        cur.execute('UPDATE PROPOSALS set hmt_active=1, hmt_deactivation_time=NULL '
                                                    'WHERE id=?', (row[0],))
                                ...
                                if not prop.db_id:
                                    ...
                                    cur.execute("INSERT INTO PROPOSALS (name, payment_start, payment_end, payment_amount,"
                                                ...
                                                " object_type, is_valid_reason, hmt_active, hmt_create_time, "
                                                " hmt_deactivation_time, hmt_voting_last_read_time)"
                                                " VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,0)",
                                                (prop.get_value('name'),
                        ...
                        # delete proposals which no longer exists in tha Hatch network
                        rows_removed = False
                        for prop_idx in reversed(range(len(self.proposals))):
                            ...
                            if not prop.marker:
                                ...
                                cur.execute("UPDATE PROPOSALS set hmt_active=0, hmt_deactivation_time=? WHERE id=?",
                ...
                log.warning('No proposals returned from hatchd.')
            ...
            log.exception('Exception wile reading proposals from Hatch network.')
            self.display_message('')
            self.errorMsg('Error while reading proposals data from the Hatch network: ' + str(e))
            raise
    ...
    def read_governance_data(self):
        try:
            ...
            self.governanceinfo = self.hatchd_intf.getgovernanceinfo()
            ...
            self.next_budget_amount = float(self.hatchd_intf.getsuperblockbudget(self.next_superblock))

            # superblocks occur every 16616 blocks (approximately 28.8 days)
            self.cur_block_height = self.hatchd_intf.getblockcount()
            ...
            self.errorMsg("Coundn't read governanceinfo from the Hatch network. "
    ...
    def get_block_timestamp(self, superblock: int):
        ...
        if ts is None:
            bhash = self.hatchd_intf.getblockhash(superblock)
            bh = self.hatchd_intf.getblockheader(bhash)
            ts = bh['time']
    ...
    def read_data_thread(self, ctrl):
        ...
        try:
            self.display_message('Connecting to Hatch daemon, please wait...')
            if not self.hatchd_intf.open():
                ...
                try:
                    ...
                    mns = self.hatchd_intf.get_masternodelist('json')
                    ...
                    if self.db_intf.db_active:
                        try:
                            ...
                            cur.execute(
                                ...
                                " is_valid_reason, hmt_active, hmt_create_time, hmt_deactivation_time, id,"
                                " hmt_voting_last_read_time, owner, title, ext_attributes_loaded, "
                                "ext_attributes_load_time "
                                "FROM PROPOSALS where hmt_active=1"
                            )
                ...
                except HatchdIndexException as e:
                    self.errorMsg(str(e))
            ...
            if not self.finishing:
                # read additional data from external sources, if configured (HatchCentral)
                proposals = []
    ...
    def read_external_attibutes(self, proposals):
        """Method reads additional proposal attributes from an external source such as HatchCentral.org/HatchNexus.org
        ...
        try:
            url = self.app_config.hatch_central_proposal_api
            if url:
                ...
                if col.column_for_vote:
                    ...
                    if mn:
                        cur.execute("SELECT proposal_id, voting_time, voting_result "
                                    ...
                                    "(SELECT 1 FROM PROPOSALS p where p.id=vr.proposal_id and p.hmt_active=1)",
                                    (mn_ident,))
                        for row in cur.fetchall():
                            if self.finishing:
                                raise CloseDialogException
    ...
    def read_voting_from_network(self, force_reload_all, proposals: List[Proposal]):
        """
        Retrieve from a Hatch daemon voting results for all defined masternodes, for all visible Proposals.
            ...
            try:
                ...
                if not self.hatchd_intf.open():
                    ...
                    try:
                        ...
                        node_info = self.hatchd_intf.rpc_call(False, False, 'getinfo')
                        ...
                        for row_idx, prop in enumerate(proposals):
                            try:
                                ...
                                tm_begin = time.time()
                                try:
                                    votes = self.hatchd_intf.rpc_call(False, False, 'gobject', getvotes_fun_name,
                                                                     prop.get_value('hash'))
                                except Exception:
                        ...
                        if cur:
                            ...
                            for prop in proposals_updated:
                                ...
                                tm_begin = time.time()
                                cur.execute("UPDATE PROPOSALS set hmt_voting_last_read_time=? where id=?",
                                            (int(time.time()), prop.db_id))
                                db_modified = True
                                db_oper_duration += (time.time() - tm_begin)
    ...
    def refresh_proposals_thread(self, ctrl):
        ...
        proposals = []
        if self.app_config.read_proposals_external_attributes:
            # select proposals for which we read additional data from external sources as HatchCentral.org
            for prop in self.proposals:
                if not prop.ext_attributes_loaded:
                    proposals.append(prop)
        if proposals and not self.finishing:
    ...
    def refresh_details_tabs(self):
        try:
            ...
            if self.current_proposal:
                ...
                if not re.match('.*hatchcentral\.ga', url.lower()):
                    dc_url = 'https://www.hatchcentral.ga/p/' + prop.get_value('name')
...
<table>
    <tbody>
        ...
        <tr class="main-row">
            ...
            <td class="padding" style="white-space:nowrap"><span>{app_utils.to_string(prop.get_value('payment_amount'))} Hatch&#47;cycle ({cycles_str}, {app_utils.to_string(prop.get_value('payment_amount_total'))} Hatch total)
...
<table>
    <tbody>
        ...
        <tr class="main-row">
            ...
            <td class="padding">{app_utils.to_string(total_amount_requested)} Hatch{total_pct_requested}</td>
        </tr>
        <tr class="main-row">
            <td class="first-col-label">Approved from the next budget:</td>
            <td class="padding">{app_utils.to_string(total_amount_approved)} Hatch{total_pct_approved}</td>
        </tr>
    </tbody>
</table>
 </body>
        ...
        for prop in proposal_list:
            ...
            for vote_idx, mn_info in enumerate(masternodes):
                ...
                try:
                    ...
                    step = 2
                    vote_sig = hatch_utils.ecdsa_sign(
                        serialize_for_sig,
                        mn_info.masternode_config.get_current_key_for_voting(self.app_config, self.hatchd_intf),
                        self.app_config.hatch_network)

                    step = 3
                    v_res = self.hatchd_intf.voteraw(
                    ...
                    if step in (1, 4):
                        ...
                        log.info('masternode_pub_key: %s' %
                                     str(hatch_utils.wif_privkey_to_pubkey(
                                         mn_info.masternode_config.get_current_key_for_voting(
                                             self.app_config, self.hatchd_intf))))
                        log.info('masternode_pub_key_hash: %s' %
                                     str(hatch_utils.pubkey_to_address(hatch_utils.wif_privkey_to_pubkey(
                                         mn_info.masternode_config.get_current_key_for_voting(
                                             self.app_config, self.hatchd_intf)), self.app_config.hatch_network)))
