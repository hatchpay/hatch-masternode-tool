...
from app_config import AppConfig, MasternodeConfig, APP_NAME_SHORT, HMN_ROLE_OWNER, HMN_ROLE_OPERATOR, InputKeyType
from app_defs import PROJECT_URL, HWType, get_note_url
from hatch_utils import bip32_path_n_to_string
from hatchd_intf import HatchdInterface, HatchdIndexException
...
class MainWindow(QMainWindow, WndUtils, ui_main_dlg.Ui_MainWindow):
    ...
    def __init__(self, app_dir):
        ...
        self.hatchd_intf = HatchdInterface(window=None,
        ...
        self.hw_session = HwSessionInfo(
            ...
            self.app_config,
            hatchd_intf=self.hatchd_intf)

        self.hatchd_info = {}
        self.is_hatchd_syncing = False
        self.hatchd_connection_ok = False
        self.connecting_to_hatchd = False
    ...
    def setupUi(self):
        ...
        self.wdg_masternode = WdgMasternodeDetails(self, self.app_config, self.hatchd_intf)
        ...
        self.hatchd_intf.initialize(self.app_config)
    ...
    def load_configuration_from_file(self, file_name: str, ask_save_changes = True,
        ...
        try:
            self.disconnect_hardware_wallet()
            hatch_network_sav = self.app_config.hatch_network
            ...
            self.editing_enabled = False
            self.configuration_to_ui()
            self.hatchd_intf.reload_configuration()
            ...
            if file_name:
                ...
                self.update_config_files_mru_menu_items()
                if hatch_network_sav != self.app_config.hatch_network:
                    self.disconnect_hardware_wallet()
    ...
    def display_window_title(self):
        ...
        if self.app_config.hatch_network == 'TESTNET':
    ...
    def get_project_config_params_thread(self, ctrl, force_check):
        ...
        try:
            response = urllib.request.urlopen(
                'https://raw.githubusercontent.com/hatchpay/hatch-masternode-tool/master/app-params.json',
    ...
    def on_action_open_settings_window_triggered(self):
        ...
        if res and dlg.get_is_modified():
            ...
            self.hatchd_intf.reload_configuration()
            if hatch_network_sav != self.app_config.hatch_network or hw_type_sav != self.app_config.hw_type:
                self.disconnect_hardware_wallet()
                self.app_config.reset_network_dependent_dyn_params()
            self.display_window_title()
            self.update_edit_controls_state()
    ...
    def show_connection_initiated(self):
        """Shows status information related to a initiated process of connection to a hatch RPC. """
        self.set_status_text1('<b>RPC network status:</b> trying %s...' % self.hatchd_intf.get_active_conn_description(), 'black')
    ...
    def show_connection_failed(self):
        ...
        self.set_status_text1('<b>RPC network status:</b> failed connection to %s' % self.hatchd_intf.get_active_conn_description(), 'red')

    def show_connection_successful(self):
        """Shows status information after successful connetion to a Hatch RPC node."""
        self.set_status_text1('<b>RPC network status:</b> OK (%s)' % self.hatchd_intf.get_active_conn_description(), 'green')

    def show_connection_disconnected(self):
        """Shows status message related to disconnection from Hatch RPC node."""
        self.set_status_text1('<b>RPC network status:</b> not connected', 'black')

    def connect_hatch_network(self, wait_for_check_finish=False, call_on_check_finished=None):
        """
        Connects do hatch daemon if not connected before and returnes if it was successful.
        ...
        def wait_for_synch_finished_thread(ctrl):
            """
            Thread waiting for hatch daemon to finish synchronizing.
            """
            mtx = QMutex()
            cond = QWaitCondition()
            try:
                ...
                while not ctrl.finish:
                    synced = self.hatchd_intf.issynchronized()
                    if synced:
                        self.is_hatchd_syncing = False
                        self.show_connection_successful()
                        break
                    mnsync = self.hatchd_intf.mnsync()
                    self.add_app_message(
                        DispMessage.HATCH_NET_CONNECTION,
                        'Hatchd is synchronizing: AssetID: %s, AssetName: %s' %
                        (str(mnsync.get('AssetID', '')), str(mnsync.get('AssetName', ''))), 'warn')
                    cond.wait(mtx, 5000)
                self.del_app_message(DispMessage.HATCH_NET_CONNECTION)
            except Exception as e:
                self.is_hatchd_syncing = False
                self.hatchd_connection_ok = False
                self.add_app_message(DispMessage.HATCH_NET_CONNECTION, str(e), 'error')
            finally:
                mtx.unlock()
                self.wait_for_hatchd_synced_thread = None

        def connect_thread(ctrl):
            """
            Test connection to hatch network inside a thread to avoid blocking GUI.
            :param ctrl: control structure to communicate with WorkerThread object (not used here)
            """
            try:
                synced = self.hatchd_intf.issynchronized()
                self.hatchd_info = self.hatchd_intf.getinfo(verify_node=True)
                self.hatchd_connection_ok = True
                if not synced:
                    logging.info("hatchd not synced")
                    if not self.is_hatchd_syncing and not (hasattr(self, 'wait_for_hatchd_synced_thread') and
                                                                  self.wait_for_hatchd_synced_thread is not None):
                        self.is_hatchd_syncing = True
                        self.wait_for_hatchd_synced_thread = self.run_thread(self, wait_for_synch_finished_thread, (),
                ...
                self.add_app_message(DispMessage.HATCH_NET_CONNECTION, err, 'error')
        ...
        def connect_finished():
            ...
            self.connecting_to_hatchd = False
            self.app_config.read_hatch_network_app_params(self.hatchd_intf)
        ...
        if self.app_config.is_config_complete():
            if not hasattr(self, 'check_conn_thread') or self.check_conn_thread is None:
                ...
                if hasattr(self, 'wait_for_hatchd_synced_thread') and self.wait_for_hatchd_synced_thread is not None:
                    if call_on_check_finished is not None:
                        # if a thread waiting for hatchd to finish synchronizing is running, call the callback function
                        call_on_check_finished()
                else:
                    self.connecting_to_hatchd = True
    ...
    def on_action_check_network_connection_triggered(self):
        self.test_hatch_network_connection()

    def test_hatch_network_connection(self):
        def connection_test_finished():

            self.action_check_network_connection.setEnabled(True)
            self.btnRegisterHmn.setEnabled(True)
            ...
            if self.hatchd_connection_ok:
                self.show_connection_successful()
                if self.is_hatchd_syncing:
                    self.infoMsg('Connection successful, but Hatch daemon is synchronizing.')
                else:
                    self.infoMsg('Connection successful.')
            else:
                if self.hatchd_intf.last_error_message:
                    self.errorMsg('Connection error: ' + self.hatchd_intf.last_error_message)
                else:
                    self.errorMsg('Connection error')

        if self.app_config.is_config_complete():
            self.action_check_network_connection.setEnabled(False)
            self.btnRegisterHmn.setEnabled(False)
            ...
            self.connect_hatch_network(call_on_check_finished=connection_test_finished)
        else:
        ...
        if not self.hw_client:
            try:
                try:
                    ...
                    if self.app_config.hatch_network == 'TESTNET':
                        # check if Hatch testnet is supported by this hardware wallet
                        found_testnet_support = False
                        if self.app_config.hw_type in (HWType.trezor, HWType.keepkey):
                            try:
                                path = hatch_utils.get_default_bip32_base_path(self.app_config.hatch_network)
                                path += "/0'/0/0"
                                path_n = hatch_utils.bip32_path_string_to_n(path)
                                addr = hw_intf.get_address(self.hw_session, path_n, False)
                                if addr and hatch_utils.validate_address(addr, self.app_config.hatch_network):
                                    found_testnet_support = True
                            except Exception as e:
                                if str(e).find('Invalid coin name') < 0:
                                    logging.exception('Failed when looking for Hatch testnet support')
                        elif self.app_config.hw_type == HWType.ledger_nano_s:
                            addr = hw_intf.get_address(self.hw_session,
                                                       hatch_utils.get_default_bip32_path(self.app_config.hatch_network))
                            if hatch_utils.validate_address(addr, self.app_config.hatch_network):
                                found_testnet_support = False

                        if not found_testnet_support:
                            url = get_note_url('HMT0002')
                            msg = f'Your hardware wallet device does not support Hatch TESTNET ' \
                ...
                try:
                    with open(file_name, 'r') as f_ptr:
                        bip44_wallet = Bip44Wallet(self.app_config.hw_coin_name, self.hw_session,
                                                   self.app_config.db_intf, self.hatchd_intf,
                            ...
                            if len(elems) >= 5 and not line.startswith('#'):
                                ...
                                def update_mn(in_mn):
                                    ...
                                    if len(ipelems) >= 2:
                                        ...
                                        in_mn.ip = mn_ipport
                                        in_mn.port = '8888'
                                    in_mn.collateralAddress = mn_hatch_addr
                        ...
                        if imported_cnt:
                            ...
                            if self.queryDlg(message=msg_text, buttons=QMessageBox.Yes | QMessageBox.No,
                                ...
                                # scan all Hatch addresses from imported masternodes for BIP32 path, starting from
                                # first standard Hatch BIP32 path
    ...
    def update_edit_controls_state(self):
        def update_fun():
            ...
            self.btnRegisterHmn.setEnabled(self.cur_masternode is not None)
    ...
    def update_mn_controls_state(self):
        if self.cur_masternode:
            enabled = self.cur_masternode.hmn_user_roles & HMN_ROLE_OWNER > 0
        else:
            enabled = False
        self.btnRegisterHmn.setEnabled(enabled)
        ...
        if self.cur_masternode:
            enabled = self.cur_masternode.hmn_user_roles & HMN_ROLE_OPERATOR > 0
        else:
            enabled = False
    ...
    def get_deterministic_tx(self, masternode: MasternodeConfig) -> Optional[Dict]:
        ...
        if masternode.hmn_tx_hash:
            try:
                protx = self.hatchd_intf.protx('info', masternode.hmn_tx_hash)
                if protx:
            ...
            if not protx:
                try:
                    ...
                    tx = self.hatchd_intf.getrawtransaction(masternode.hmn_tx_hash, 1, skip_cache=True)
                    confirmations = tx.get('confirmations', 0)
                    if confirmations < 3:
                        # in this case hmn tx should have been found by the 'protx info' call above;
                        # it hasn't been, so it is no longer valid a protx transaction
                        ptx = tx.get('proRegTx')
                        if ptx:
                            protx = {
                                'proTxHash': masternode.hmn_tx_hash,
    ...
    def get_masternode_status_description_thread(self, ctrl, masternode: MasternodeConfig):
        ...
        if self.hatchd_connection_ok:
            ...
            self.hatchd_intf.get_masternodelist('json', data_max_age=30)  # read new data from the network
                                                                         # every 30 seconds
            if collateral_id:
                mn_info = self.hatchd_intf.masternodes_by_ident.get(collateral_id)
            else:
                mn_info = self.hatchd_intf.masternodes_by_ip_port.get(ip_port)

            block_height = self.hatchd_intf.getblockcount()
            hmn_tx = self.get_deterministic_tx(masternode)
            if hmn_tx:
                hmn_tx_state = hmn_tx.get('state')
            else:
                hmn_tx_state = {}
                hmn_tx = {}
            ...
            if mn_info:
                ...
                if hmn_tx_state:
                    mn_ident = str(hmn_tx.get('collateralHash')) + '-' + str(hmn_tx.get('collateralIndex'))
                    mn_ip_port = hmn_tx_state.get('service')
                else:
                    mn_ident = None
                    mn_ip_port = None

            status_color = 'black'
            if mn_info or hmn_tx:
                mn_status = ''
                ...
                if hmn_tx:
                    pose = hmn_tx_state.get('PoSePenalty', 0)
                    if pose:
                        mn_status += (', ' if mn_status else '') + 'PoSePenalty: ' + str(pose)
                        status_color = 'red'

                    oper_pub_key = hmn_tx_state.get('pubKeyOperator', '')
                    if re.match('^0+$', oper_pub_key):
                        no_operator_pub_key = True

                    service = hmn_tx_state.get('service', '')
                ...
                if masternode not in self.mns_user_refused_updating:
                    ...
                    if not masternode.collateralAddress and masternode.hmn_user_roles & HMN_ROLE_OWNER:
                        missing_data.append('collateral address')
                    if hmn_tx and masternode.hmn_tx_hash != hmn_tx.get('proTxHash'):
                ...
                if not collateral_tx_mismatch:
                    if masternode.hmn_user_roles & HMN_ROLE_OWNER:

                        # check outputs of the collateral transaction
                        tx_json = self.hatchd_intf.getrawtransaction(masternode.collateralTx, 1)
                        if tx_json:
                ...
                if hmn_tx:
                    hmn_hash = hmn_tx.get('proTxHash')
                    if hmn_hash and masternode.hmn_tx_hash != hmn_hash:
                        if update_mn_info:
                            masternode.hmn_tx_hash = hmn_tx.get('proTxHash')
                            mn_data_modified = True

                    if hmn_tx_state:
                        owner_address_network = hmn_tx_state.get('ownerAddress')
                        owner_address_cfg = masternode.get_hmn_owner_public_address(self.app_config.hatch_network)
                        if owner_address_network and owner_address_cfg and owner_address_network != owner_address_cfg:
                            ...
                            logging.warning(
                                ...
                                f'address from the app configuration: {owner_address_cfg}, address from the Hatch '
                                f'network: {owner_address_network}')

                        voting_address_network = hmn_tx_state.get('votingAddress')
                        voting_address_cfg = masternode.get_hmn_voting_public_address(self.app_config.hatch_network)
                        if voting_address_network and voting_address_cfg and voting_address_network != voting_address_cfg:
                            ...
                            logging.warning(
                                ...
                                f'address from the app configuration: {voting_address_cfg}, address from the Hatch '
                                f'network: {voting_address_network}')

                        if not no_operator_pub_key:
                            operator_pubkey_network = hmn_tx_state.get('pubKeyOperator')
                            operator_pubkey_cfg = masternode.get_hmn_operator_pubkey()
                            if operator_pubkey_network and operator_pubkey_cfg and \
                                ...
                                logging.warning(
                                    ...
                                    f'pubkey from the app configuration: {operator_pubkey_cfg}, pubkey from the Hatch '
                ...
                if masternode == self.cur_masternode:
                    ...
                    payout_address = hmn_tx_state.get('payoutAddress','')
                    ...
                    try:
                        if collateral_address:
                            collateral_bal = self.hatchd_intf.getaddressbalance([collateral_address])
                        ...
                        if collateral_address != payout_address and payout_address:
                            payout_bal = self.hatchd_intf.getaddressbalance([payout_address])
                        ...
                        operator_reward = float(hmn_tx.get('operatorReward', 0))
                        if operator_reward:
                            operator_payout_addr = hmn_tx_state.get('operatorPayoutAddress', '')
                        ...
                        if mn_info:
                            if mn_info.lastpaidtime > time.time() - 3600 * 24 * 365:
                                # fresh hmns have lastpaidtime set to some day in the year 2014
                                lastpaid_ts = mn_info.lastpaidtime
                        else:
                            paid_block = hmn_tx_state.get('lastPaidHeight')
                            if paid_block:
                                bh = self.hatchd_intf.getblockhash(paid_block)
                                blk = self.hatchd_intf.getblockheader(bh, 1)
                    ...
                    if hmn_tx and not hmn_tx.get('confirmations'):
                        warnings.append('<td class="warning" colspan="2">ProRegTx not yet confirmed</td>')
                    else:
                        if self.hatchd_intf.is_protx_update_pending(self.cur_masternode.hmn_tx_hash):
                    ...
                    if owner_public_address_mismatch and not skip_data_mismatch:
                        errors.append('<td class="error" colspan="2">Owner Hatch address mismatch</td>')
                    ...
                    if voting_public_address_mismatch and not skip_data_mismatch:
                        errors.append('<td class="error" colspan="2">Voting Hatch address mismatch</td>')
                    if not hmn_tx:
            ...
            status = '<span style="color:red">Problem with connection to hatchd.</span>'
    ...
    def on_btnRefreshMnStatus_clicked(self):
        ...
        self.connect_hatch_network(wait_for_check_finish=True)
        if self.hatchd_connection_ok:
            try:
            ...
            enable_buttons()
            self.lblMnStatus.setText('')
            self.errorMsg('Hatch daemon not connected')
    ...
    def on_action_transfer_funds_for_cur_mn_triggered(self):
        ...
        if self.cur_masternode:
            ...
            if not self.cur_masternode.collateralBip32Path:
                ...
                self.errorMsg("Enter the masternode collateral Hatch address. You can use the 'left arrow' "
    ...
    def on_action_sign_message_with_owner_key_triggered(self):
        if self.cur_masternode:
            pk = self.cur_masternode.hmn_owner_private_key
            if not pk:
    ...
    def on_action_sign_message_with_voting_key_triggered(self):
        if self.cur_masternode:
            pk = self.cur_masternode.hmn_voting_private_key
            if not pk:
    ...
    def on_action_open_proposals_window_triggered(self):
        ui = ProposalsDlg(self, self.hatchd_intf)
        ui.exec_()
    ...
    @pyqtSlot(bool)
    def on_btnRegisterHmn_clicked(self, enabled):
        reg_dlg = None

        def on_proregtx_finished(masternode: MasternodeConfig):
            nonlocal reg_dlg, self
            try:
                if self.cur_masternode.hmn_tx_hash != reg_dlg.hmn_reg_tx_hash or \
                        self.cur_masternode.hmn_owner_key_type != reg_dlg.hmn_owner_key_type or \
                        (self.cur_masternode.hmn_owner_key_type == InputKeyType.PRIVATE and
                         self.cur_masternode.hmn_owner_private_key != reg_dlg.hmn_owner_privkey) or \
                        (self.cur_masternode.hmn_owner_key_type == InputKeyType.PUBLIC and
                         self.cur_masternode.hmn_owner_address != reg_dlg.hmn_owner_address) or \
                        self.cur_masternode.hmn_operator_key_type != reg_dlg.hmn_operator_key_type or \
                        (self.cur_masternode.hmn_operator_key_type == InputKeyType.PRIVATE and
                         self.cur_masternode.hmn_operator_private_key != reg_dlg.hmn_operator_privkey) or \
                        (self.cur_masternode.hmn_operator_key_type == InputKeyType.PUBLIC and
                         self.cur_masternode.hmn_operator_public_key != reg_dlg.hmn_operator_pubkey) or \
                        self.cur_masternode.hmn_voting_key_type != reg_dlg.hmn_voting_key_type or \
                        (self.cur_masternode.hmn_voting_key_type == InputKeyType.PRIVATE and
                         self.cur_masternode.hmn_voting_private_key != reg_dlg.hmn_voting_privkey) or \
                        (self.cur_masternode.hmn_voting_key_type == InputKeyType.PUBLIC and
                         self.cur_masternode.hmn_voting_address != reg_dlg.hmn_voting_address):

                    self.cur_masternode.hmn_tx_hash = reg_dlg.hmn_reg_tx_hash

                    self.cur_masternode.hmn_owner_key_type = reg_dlg.hmn_owner_key_type
                    if self.cur_masternode.hmn_owner_key_type == InputKeyType.PRIVATE:
                        self.cur_masternode.hmn_owner_private_key = reg_dlg.hmn_owner_privkey
                    else:
                        self.cur_masternode.hmn_owner_address = reg_dlg.hmn_owner_address
                        self.cur_masternode.hmn_owner_private_key = ''

                    self.cur_masternode.hmn_operator_key_type = reg_dlg.hmn_operator_key_type
                    if self.cur_masternode.hmn_operator_key_type == InputKeyType.PRIVATE:
                        self.cur_masternode.hmn_operator_private_key = reg_dlg.hmn_operator_privkey
                    else:
                        self.cur_masternode.hmn_operator_public_key = reg_dlg.hmn_operator_pubkey
                        self.cur_masternode.hmn_operator_private_key = ''

                    self.cur_masternode.hmn_voting_key_type = reg_dlg.hmn_voting_key_type
                    if self.cur_masternode.hmn_voting_key_type == InputKeyType.PRIVATE:
                        self.cur_masternode.hmn_voting_private_key = reg_dlg.hmn_voting_privkey
                    else:
                        self.cur_masternode.hmn_voting_address = reg_dlg.hmn_voting_address
                        self.cur_masternode.hmn_voting_private_key = ''
        ...
        if self.cur_masternode:
            reg_dlg = reg_masternode_dlg.RegMasternodeDlg(self, self.app_config, self.hatchd_intf, self.cur_masternode,
    ...
    def update_registrar(self, show_upd_payout: bool, show_upd_operator: bool, show_upd_voting: bool):
        ...
        if self.cur_masternode:
            upd_dlg = upd_mn_registrar_dlg.UpdMnRegistrarDlg(
                self, self.app_config, self.hatchd_intf, self.cur_masternode,
