...
import app_config
import app_cache
from app_config import AppConfig, HatchNetworkConnectionCfg
from hatchd_intf import HatchdInterface, control_rpc_call
...
class ConfigDlg(QDialog, Ui_ConfigDlg, WndUtils):
    def __init__(self, parent, app_config: AppConfig):
        ...
        # list of connections from self.local_config.hatch_net_configs split on separate lists for mainnet and testnet
        self.connections_mainnet = []
        self.connections_testnet = []
        self.connections_current = None
        self.current_network_cfg : Optional[HatchNetworkConnectionCfg] = None
    ...
    @pyqtSlot(int)
    def on_cboHatchNetwork_currentIndexChanged(self, index):
        """Executed after changing configuration between MAINNET and TESTNET."""
        if not self.disable_cfg_update:
            if index == 0:
                self.connections_current = self.connections_mainnet
                self.local_config.hatch_network = 'MAINNET'
            else:
                self.connections_current = self.connections_testnet
                self.local_config.hatch_network = 'TESTNET'
            self.display_connection_list()
            self.set_modified()
            self.lstConns.setCurrentRow(0)
        ...
        try:
            ...
            if isinstance(conns, list) and len(conns):
                ...
                if self.queryDlg('Do you really want to import connection(s) from clipboard?',
                    ...
                    self.local_config.hatch_net_configs.clear()
                    self.local_config.hatch_net_configs.extend(self.connections_mainnet)
                    self.local_config.hatch_net_configs.extend(self.connections_testnet)

                    added, updated = self.local_config.import_connections(
                        conns, force_import=True, limit_to_network=self.local_config.hatch_network)
                    for cfg in added:
                        cfg.enabled = True
    ...
    def on_btnRestoreDefault_clicked(self, enabled):
        if self.queryDlg('Do you really want to restore default connection(s)?',
                         buttons=QMessageBox.Yes | QMessageBox.Cancel,
                         default_button=QMessageBox.Yes, icon=QMessageBox.Information) == QMessageBox.Yes:
            cfgs = self.local_config.decode_connections(default_config.hatchd_default_connections)
            if cfgs:
                # update the main list containing connections configuration from separate  lists dedicated
                # to mainnet and testnet - it'll be used by the import_connection method
                self.local_config.hatch_net_configs.clear()
                ...
                added, updated = self.local_config.import_connections(
                    cfgs, force_import=True, limit_to_network=self.local_config.hatch_network)
                self.connections_current.extend(added)
                if added or updated:
                    row_selected = self.lstConns.currentRow()
                    self.display_connection_list()
    ...
    def on_action_new_connection_triggered(self):
        ...
        cfg.testnet = True if self.cboHatchNetwork.currentIndex() == 1 else False
    ...
    def on_chbRandomConn_toggled(self, checked):
        if not self.disable_cfg_update:
            self.local_config.random_hatch_net_config = checked
            self.set_modified()
    ...
    def update_connection_details_ui(self):
        ...
        try:
            ...
            self.chbRandomConn.setChecked(self.local_config.random_hatch_net_config)
        finally:
    ...
    def on_btnSshReadRpcConfig_clicked(self):
        """Read the configuration of a remote RPC node from the node's hatch.conf file."""
        if self.current_network_cfg:
            ...
            if not ok or not username:
                return
            from hatchd_intf import HatchdSSH
            ssh = HatchdSSH(host, int(port), username, auth_method=auth_method, private_key_path=private_key_path)
            try:
                if ssh.connect():
                    hatchd_conf = ssh.find_hatchd_config()
                    self.disable_cfg_update = True
                    if isinstance(hatchd_conf, tuple) and len(hatchd_conf) >= 3:
                        if not hatchd_conf[0]:
                            self.infoMsg('Remore Hatch daemon seems to be shut down')
                        elif not hatchd_conf[1]:
                            self.infoMsg('Could not find remote hatchd.conf file')
                        else:
                            file = hatchd_conf[2]
                            rpcuser = file.get('rpcuser', '')
                            rpcpassword = file.get('rpcpassword', '')
                            rpcport = file.get('rpcport', '8884')
                            modified = False
                            if rpcuser:
                            ...
                            if file.get('server', '1') == '0':
                                self.warnMsg("Remote hatch.conf parameter 'server' is set to '0', so RPC interface will "
                                             "not work.")
                            if not rpcuser:
                                self.warnMsg("Remote hatch.conf parameter 'rpcuser' is not set, so RPC interface will  "
                                             "not work.")
                            if not rpcpassword:
                                self.warnMsg("Remote hatch.conf parameter 'rpcpassword' is not set, so RPC interface will  "
                                             "not work.")
                        self.update_connection_details_ui()
                    elif isinstance(hatchd_conf, str):
                        self.warnMsg("Couldn't read remote hatchd configuration file due the following error: " +
                                     hatchd_conf)
                    ssh.disconnect()
            except Exception as e:
                self.errorMsg(str(e))
    ...
    def on_btnTestConnection_clicked(self):
        if self.current_network_cfg:
            ...
            hatchd_intf = HatchdInterface(window=self)
            hatchd_intf.initialize(self.local_config, connection=self.current_network_cfg,
                                  for_testing_connections_only=True)
            try:
                info = hatchd_intf.getinfo(verify_node=True)
                if info:
                    try:
                        ret = hatchd_intf.rpc_call(True, False, "checkfeaturesupport", "enhanced_proxy")
                    except Exception as e:
                        ret = None
