...
class CmdConsoleDlg(QDialog, ui_cmd_console_dlg.Ui_CmdConsoleDlg):
    ...
    def print_help(self):
        ...
        <b>set loglevel ["module-name":"log-level",...]</b>
          ...
          Example: set loglevel all:info,hmt.bip44_wallet:debug  
    ...
    def rpc_command(self, command: str, *args):
        if self.main_dlg.hatchd_intf:
            ret = self.main_dlg.hatchd_intf.rpc_call(False, True, command, *args)
            try:
            ...
            WndUtils.errorMsg('Not connected to a Hatch node')
            return False
