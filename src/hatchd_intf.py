...
log = logging.getLogger('hmt.hatchd_intf')
...
class HatchdSSH(object):
    ...
    def find_hatchd_config(self):
        """
        Try to read configuration of remote hatch daemon. In particular we need parameters concering rpc
        configuration.
        :return: tuple (hatchd_running, hatchd_config_found, hatchd config file contents as dict)
                or error string in error occured
        """
        hatchd_running = False
        hatchd_config_found = False
        if not self.ssh:
            raise Exception('SSH session not ready')
        try:
            # find hatchd process id if running
            try:
                pids = self.remote_command('ps -C "hatchd" -o pid')
            except UnknownError:
                raise Exception('is hatchd running on the remote machine?')
            pid = None
            ...
            if pid:
                hatchd_running = True
                # using hatchd pid find its executable path and then .hatchcore directory and finally hatch.conf file
                ...
                if executables and len(executables) >= 1:
                    ...
                    if len(elems) == 2:
                        ...
                        hatchd_dir = os.path.dirname(executable)
                        hatch_conf_file = hatchd_dir + '/.hatchcore/hatch.conf'
                        conf_lines = []
                        try:
                            conf_lines = self.remote_command('cat ' + hatch_conf_file)
                        except Exception as e:
                            # probably error no such file or directory
                            # try to read hatchd's cwd + cmdline
                            cwd_lines = self.remote_command('ls -l /proc/' + str(pid) + '/cwd')
                            if cwd_lines:
                                elems = cwd_lines[0].split('->')
                                if len(elems) >= 2:
                                    cwd = elems[1]
                                    hatch_conf_file = cwd + '/.hatchcore/hatch.conf'
                                    try:
                                        conf_lines = self.remote_command('cat ' + hatch_conf_file)
                                    except Exception as e:
                                        # second method did not suceed, so assume, that conf file is located
                                        # i /home/<username>/.hatchcore directory
                                        hatch_conf_file = '/home/' + self.username + '/.hatchcore/hatch.conf'
                                        conf_lines = self.remote_command('cat ' + hatch_conf_file)
                        ...
                        hatchd_config_found = True
            return hatchd_running, hatchd_config_found, config
        except Exception as e:
            return str(e)
...
class HatchdIndexException(JSONRPCException):
    """
    Exception for notifying, that hatch daemon should have indexing option tuned on
    """
    def __init__(self, parent_exception):
        JSONRPCException.__init__(self, parent_exception.error)
        self.message = self.message + \
                       '\n\nMake sure the hatch daemon you are connecting to has the following options enabled in ' \
                       'its hatch.conf:\n\n' + \
                       'addressindex=1\n' + \
                       'spentindex=1\n' + \
                       'timestampindex=1\n' + \
                       'txindex=1\n\n' + \
                       'Changing these parameters requires to execute hatchd with "-reindex" option (linux: ./hatchd -reindex)'
                        ...
                        try:
                            if encrypt_rpc_arguments:
                                ...
                                if pubkey:
                                    ...
                                    args = (args[0], 'HMTENCRYPTEDV1') + tuple(encrypted_parts)
                                    log.info(
                            ...
                            if last_conn_reset_time:
                                raise HatchdConnectionError(e)  # switch to another config if possible
                            else:
                                last_exception = e
                            ...
                            # exceptions raised most likely by not functioning hatchd node; try to switch to another node
                            # if there is any in the config
                            log.warning('Error while calling of "' + str(func) + ' (3)". Details: ' + str(e))
                            raise HatchdConnectionError(e)
...
class HatchdInterface(WndUtils):
    def __init__(self, window,
        ...
        self.cur_conn_def: Optional['HatchNetworkConnectionCfg'] = None
    ...
    def load_data_from_db_cache(self):
        ...
        try:
            ...
            cur.execute("SELECT id, ident, status, payee, last_seen, active_seconds,"
                        " last_paid_time, last_paid_block, IP, queue_position from MASTERNODES where hmt_active=1")
    ...
    def switch_to_next_config(self):
        """
        If there is another hatchd config not used recently, switch to it. Called only when there was a problem
    ...
    def open(self):
        """
        Opens connection to hatch RPC. If it fails, then the next enabled conn config will be used, if any exists.
        ...
        try:
            if not self.cur_conn_def:
                raise Exception('There is no connections to Hatch network enabled in the configuration.')
            ...
            while True:
                try:
                    ...
                    return False
                        ...
                        NoValidConnectionsError) as e:
                    # exceptions raised by not likely functioning hatchd node; try to switch to another node
                    # if there is any in the config
                    if not self.switch_to_next_config():
    ...
    def open_internal(self):
        """
        Try to establish connection to hatch RPC daemon for current connection config.
        ...
        if not self.active:
            ...
            if self.cur_conn_def.use_ssh_tunnel:
                # RPC over SSH
                if self.ssh is None:
                    self.ssh = HatchdSSH(self.cur_conn_def.ssh_conn_cfg.host, self.cur_conn_def.ssh_conn_cfg.port,
        ...
        if self.open():
            ...
            if verify_node:
                ...
                if self.app_config.is_testnet() and not node_under_testnet:
                    raise Exception('This RPC node works under Hatch MAINNET, but your current configuration is '
                                    'for TESTNET.')
                elif self.app_config.is_mainnet() and node_under_testnet:
                    raise Exception('This RPC node works under Hatch TESTNET, but your current configuration is '
                                    'for MAINNET.')
            return info
        else:
            raise Exception('Not connected')
    ...
    def read_protx_list(self):
        last_read_time = app_cache.get_value(f'ProtxLastReadTime_{self.app_config.hatch_network}', 0, int)
    ...
    def get_masternodelist(self, *args, data_max_age=MASTERNODES_CACHE_VALID_SECONDS,
        ...
        """
        Returns masternode list, read from the Hatch network or from the internal cache.
        ...
        if self.open():
            ...
            if len(args) == 1 and args[0] == 'json':
                last_read_time = app_cache.get_value(f'MasternodesLastReadTime_{self.app_config.hatch_network}', 0, int)
                   ...
                   int(time.time()) - last_read_time < data_max_age:
                    ...
                    try:
                            ...
                            if not existing_mn:
                                ...
                                if self.db_intf.db_active:
                                    cur.execute(
                                        ...
                                        " registered_height, hmt_active, hmt_create_time, queue_position) "
                                        "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
                        ...
                        for mn_index in reversed(range(len(self.masternodes))):
                            ...
                            if not mn.marker:
                                if self.db_intf.db_active:
                                    cur.execute("UPDATE MASTERNODES set hmt_active=0, hmt_deactivation_time=?"
                                                "WHERE ID=?",
                        ...
                        app_cache.set_value(f'MasternodesLastReadTime_{self.app_config.hatch_network}', int(time.time()))
    ...
    @control_rpc_call
    def checkfeaturesupport(self, feature_name: str, hmt_version: str, *args) -> Dict:
        if self.open():
            return self.proxy.checkfeaturesupport(feature_name, hmt_version)
        else:
            raise Exception('Not connected')
