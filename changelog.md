...
for the operator and Hatch addresses for the owner and voting.
- Main window: the possibility of displaying private keys in the form 
of: Hatch address, public key and public key hash (for diagnostics).
...
in the `protx prepare` call (added in Hatch Core rc11).
...
**Added**
- Support for Hatch v13 - Deterministic Masternode List (protocol 70213)
...
**Added**
- Hatch daemon v12.3 support (protocol 70209/70210)
...
- Moved to the official Trezor Insight API for Hatch (https://hatch-bitcore1.trezor.io/api/).
...
- The connection was incorrectly shown as successful for proxy connections when nginx was running, but the Hatch daemon was not.
...
- Changed the domain for "public" nodes to something more relevant: hatch-masternode-tool.org.
- Changed the TCP port number for *alice* and *luna* "public" RPC nodes to 443 - the official HTTPS port.
- Added a new "public" RPC node: suzy.hatch-masternode-tool.org:443.
